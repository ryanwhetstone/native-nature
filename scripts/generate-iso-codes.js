// Mapping of ISO 3166-1 alpha-3 to alpha-2 codes
const iso3to2 = {
...
const fs = require('fs');
const countriesFile = fs.readFileSync('../lib/countries.ts', 'utf8');

// Find all country entries and add isoCode2
const updated = countriesFile.replace(
  /(\w+): { isoCode: "(\w+)", name: "([^"]+)", slug: "([^"]+)" }/g,
  (match, key, isoCode, name, slug) => {
    const isoCode2 = iso3to2[isoCode] || "XX";
    return `${key}: { isoCode: "${isoCode}", isoCode2: "${isoCode2}", name: "${name}", slug: "${slug}" }`;
  }
);

fs.writeFileSync('../lib/countries.ts', updated);
console.log('Updated countries.ts with 2-letter ISO codes');
