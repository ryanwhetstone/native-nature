...
async function getPlants(placeId: number, taxonId: number) {
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch mammals');
    }
...
export default async function MammalsPage({
  ...
  const taxonId = 40151; // Mammals (Mammalia)
  ...
  return (
    <main className="min-h-screen p-8">
      ...
      <h2 className="text-2xl text-gray-600 mb-8">Mammals</h2>
      
      <section>
        {plants.length === 0 ? (
          <p className="text-gray-600">Loading mammal data...</p>
        ) : (
